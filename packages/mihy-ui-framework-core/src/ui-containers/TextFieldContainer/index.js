...
class TextFieldContainer extends React.Component {
  render() {
    let { label, placeholder, jsonPath, iconObj, value,dropdownData,data, optionValue, optionLabel,sourceJsonPath, ...rest } = this.props;
...
const mapStateToProps = (state, ownprops) => {
  const { jsonPath, value, select, data, optionValue, optionLabel,sourceJsonPath } = ownprops;
  ...
  if (select) {
    ...
    if (data && data.length > 0) {
      ...
      dropdownData=constructDropdown(get(preparedFinalObject, sourceJsonPath,[]));
    }
  }
